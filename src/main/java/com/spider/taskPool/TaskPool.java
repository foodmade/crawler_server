...
public class TaskPool {
    ...
    public static TaskData getSubTask(String task_type){
        ...
        try{
            ...
            if(!taskIndexMap.containsKey(curQueryTaskNode.getQuery_id())){
                logger.error("异常的query_id:"+curQueryTaskNode.getQuery_id());
                return null;
            }
            ...
            logger.error("获取taskData出现异常 e:"+e.getMessage());
        }finally {
    ...
    public static void closeTask(String task_type,String task_id,String query_id){
        if(CommonUtils.isEmpty(task_id) || CommonUtils.isEmpty(task_type) || CommonUtils.isEmpty(query_id)){
            logger.error("关闭taskData时,任务类型和任务id都不能为空");
            return;
        }
        if(!taskIndexMap.containsKey(query_id)){
            logger.error("异常的query_id:"+query_id +" 在任务索引表里未找到,任务关闭失败 query_id:"+query_id +" task_id:"+task_id);
            return;
        }
        QueryTaskNode queryTaskNode = taskIndexMap.get(query_id);
        if(queryTaskNode == null || queryTaskNode.getQuery_id() == null){
            logger.error("获取到的任务节点为空 ,关闭任务失败 query_id:"+query_id+" task_id:"+task_id);
            return;
        }
    ...
    public static void closeQueryNode(String task_type,String query_id){
        if(CommonUtils.isEmpty(task_type) || CommonUtils.isEmpty(query_id)){
            logger.error("关闭QueryNode时,任务类型和任务id都不能为空");
            return;
        }
        ConcurrentLinkedQueue<QueryTaskNode> queue = taskPool.get(task_type);
        if(queue == null || queue.isEmpty()){
            logger.error("关闭QueryNode时,任务池中不存在query_id:"+query_id+" 的任务,关闭失败");
            return;
        }
        QueryTaskNode queryTaskNode = taskIndexMap.get(query_id);
        taskIndexMap.remove(query_id);
        queue.remove(queryTaskNode);
        logger.info("关闭QueryNode成功 query_id:"+query_id +"task_type:"+task_type);
    }
    ...
    public static void addSubTask(TaskParams taskParams){
        if(taskParams == null || taskParams.getMaxPage() <= 0){
            logger.error("创建任务失败,任务最大页码异常或者任务参数为空");
            return;
        }
        String task_type = taskParams.getTask_type();
        if(CommonUtils.isEmpty(task_type)){
            logger.error("创建任务时,task_type不能为空 task_type:"+task_type);
            return;
        }
        if(taskParams.getChildCateInfo() == null){
            logger.error("创建任务时,childCateInfo不能为空");
            return;
        }
        ...
        addTaskToPool(queryTaskNode,taskParams);
    }

    private static void forechTaskData(TaskParams taskParams,QueryTaskNode queryTaskNode,
                                       HashMap<String,Integer> sourceCateIdMap,String sourceCateId){
        for (int i = 1;i<=taskParams.getMaxPage();i++){
            ...
            taskData.setTask_type(taskParams.getTask_type());
            taskData.setPage(i);
            ...
            if(sourceCateIdMap != null && !sourceCateIdMap.isEmpty()){
                taskData.setCateid(sourceCateIdMap.get(sourceCateId));
    ...
    private static void addTaskToPool(QueryTaskNode queryTaskNode, TaskParams taskParams){
        long start = System.currentTimeMillis();
        HashMap<String,Integer> sourceCateIdMap = taskParams.getChildCateInfo();
        if(sourceCateIdMap == null || sourceCateIdMap.isEmpty()){
        ...
        logger.debug("创建【"+taskParams.getMaxPage()+"】条任务 task_type:【"+task_type+"】 耗时:【"+(System.currentTimeMillis()-start)+"】");
    }
    ...
    public static void appendSubTaskToNode(QueryTaskNode queryTaskNode, TaskParams taskParams){
        ...
        if(CommonUtils.isEmpty(queryTaskNode.getQuery_id())){
            logger.error("大任务节点初始结构异常，queryId不能为空");
            return;
        }
        addTaskToPool(queryTaskNode,taskParams);
    }
    ...
    public static QueryInfo getQueryInfo(String query_id,String task_type){
        if(CommonUtils.isEmpty(query_id) || CommonUtils.isEmpty(task_type)){
            logger.error("获取任务节点详情失败,必须参数不全 query_id:"+query_id+"  task_type:"+task_type);
            return null;
        }
        QueryInfo queryInfo = new QueryInfo();
        if(!taskIndexMap.containsKey(query_id)){
            logger.error("获取任务节点详情失败,不存在任务索引表的query_id:"+query_id);
            return queryInfo;
        }
        QueryTaskNode queryTaskNode = taskIndexMap.get(query_id);
        if(queryTaskNode == null){
