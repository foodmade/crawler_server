...
public class QueryTaskNode {
    ...
    private static Logger logger = Logger.getLogger(QueryTaskNode.class);
    ...
    public void addTask(TaskData taskData){
        if(taskData == null){
            logger.error("不能添加空任务");
            return;
        }
        if(taskDataMap.containsKey(taskData.getTask_id())){
            logger.error("任务已存在,不能重复添加 task_id:"+taskData.getTask_id());
            return;
        }
    ...
    public void closeTask(String task_id){
        if(task_id == null || task_id.equals("")){
            logger.error("异常的任务id task_id:"+task_id);
            return;
        }
        if(!taskDataMap.containsKey(task_id)){
            logger.error("不存在的task_id,任务关闭失败 task_id:"+task_id);
            return;
        }
        TaskData taskData = taskDataMap.get(task_id);

        if(taskData == null){
            logger.error("从索引表中获取到的任务为空 task_id:"+task_id);
            return;
        }
        ...
        logger.info("关闭任务成功 task_id:"+task_id+" 剩餘任務:"+unOverTaskQueue.size());
    }
    ...
    TaskData getUnOverTask(){
        if(unOverTaskQueue.isEmpty()){
            logger.debug("任务池已无任务 task_type:"+this.task_type);
            return null;
        }
        ...
        for (TaskData data : unOverTaskQueue){
            ...
            if(data.getStatus().equals(TaskConst.LOGING_EXECUTE)){          //说明任务是处于执行状态  需要判断任务是否超时,是则继续交给爬虫执行 反之跳过
                ...
                if( 1==1 ||(data.getTimestamp() + 5000) >= timeStamp){               //说明距离上次取任务时间已经超过5S 可以判定上次爬虫执行任务已经失败,进行重试
                    ...
                    if(data.getRetry() >= 5){
                        logger.error("task_id:"+data.getTask_id() + " 已经重试:"+data.getRetry()+ "次 移除任务");
